...
export class AllocationComponent implements OnInit {
  ...
  fetchData() {
    this.inlineService.fetchCombine(this.RfpNum).subscribe(
      (response: any) => {
        console.log('Fetched Data:', response);
        // Extract the phase from the 'phase' field
        this.uniqueHeader = response
          .map((item: any) => {
            const phase = item.phase?.S;
            if (phase) {
              return phase.trim();  // Return the trimmed phase name
            }
            return '';
          })
          .filter((value: string, index: number, self: string[]) => 
            value && self.indexOf(value) === index);  // Filter out duplicates and empty values
        console.log('Unique Header (Phases):', this.uniqueHeader);
      },
      error => {
