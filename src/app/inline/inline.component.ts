...
export class InlineComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.inlineService.fetchCombine(this.rfp_no).subscribe((response: any) => {
      ...
      this.existingData = response.map((item: any) => {
        const [phase, size, complexity] = item.referenceIndex.S.split('-');
        return {
          ...item,
          phase: { S: phase },
          size: { S: size },
      ...
      this.total = this.existingData.reduce((acc: number, item: any) => acc + parseInt(item.totalEsti.N), 0);
      this.roundsum = this.total;
  ...
  onPhaseChangeEdit(event: any) {
    ...
    const phaseData = this.MetricsList.filter(item => item.phase.S === phase);
    this.availableSizes = phaseData.map(item => item.size.S);
  ...
  onSizeChangeEdit(event: any) {
    ...
    const sizeData = this.MetricsList.filter(item => item.phase.S === phase && item.size.S === size);
    this.availableComplexities = sizeData.map(item => item.complexity.S);
    this.editedItem.complexity.S = null;
